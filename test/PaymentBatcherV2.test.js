...
const PaymentBatcher = artifacts.require("./PaymentBatcherV2.sol");
const BrokenContract = artifacts.require("./BrokenContract.sol");
const chai = require("./setupchai.js");
const BN = web3.utils.BN;
const expect = chai.expect;

///web3.utils.padLeft('0x3456ff', 20); ToDo::change it
...
contract("PaymentBatcherV2", accounts => {
    ...
    it("Batch ether to several addresses", async () => {   
      ...
      let result = await PaymentBatcherInstance.transferEtherToMany([clientAccount1 + zeroFill(withdrAmount3.toString(16), 24), clientAccount2 + zeroFill(withdrAmount1.toString(16), 24)], 21000, {from: ownerAccount,});
    ...
    it("Contract ether balance is less than sum of all sending ethers", async () => {   
      ...
      let prevBalanceOfClientAccount1 = await web3.eth.getBalance(clientAccount1);
      let prevBalanceOfClientAccount2 = await web3.eth.getBalance(clientAccount2);
      return expect(PaymentBatcherInstance.transferEtherToMany([clientAccount1 + zeroFill(withdrAmount1.toString(16), 24), clientAccount1 + zeroFill(withdrAmount2.toString(16), 24), clientAccount2 + zeroFill(withdrAmount3.toString(16), 24)], 21000, {from: ownerAccount,})).to.be.eventually.rejected;
    });
    ...
    it("Batch ether to several addresses with skipping contract without recieve", async () => {   
      ...
      let prevBalanceOfClientAccount2 = await web3.eth.getBalance(clientAccount2);
      let result = await PaymentBatcherInstance.transferEtherToMany([BrokenContractInstance.address + zeroFill(withdrAmount3.toString(16), 24), clientAccount2 + zeroFill(withdrAmount3.toString(16), 24)], 21000, {from: ownerAccount,});
    ...
    it("NOT owner cannot Batch ether", async () => {   
      await PaymentBatcherInstance.sendTransaction({from: clientAccount1, value: (depositAmount)});
      return expect(PaymentBatcherInstance.transferEtherToMany([clientAccount1 + zeroFill(withdrAmount2.toString(16), 24), clientAccount2 + zeroFill(withdrAmount3.toString(16), 24)], 21000, {from: clientAccount1,})).to.be.eventually.rejected;
    });
    ...
    it("Batch tokens to several accounts", async () => {   
      ...
      let result = await PaymentBatcherInstance.transferTokensToMany(SimpleTokenInstance.address, [clientAccount1 + zeroFill(withdrAmount1.toString(16), 24), clientAccount2 + zeroFill(withdrAmount3.toString(16), 24)], {from: ownerAccount});
    ...
    it("Batch several allowed tokens to several accounts", async () => {
      ...
      result = await PaymentBatcherInstance.transferTokensToMany(AnotherTokenInstance.address, [clientAccount1+ zeroFill(withdrAmount1.toString(16), 24), clientAccount2 + zeroFill(withdrAmount3.toString(16), 24)], {from: ownerAccount});
      console.log(`GasUsed: ${result.receipt.gasUsed}`);
    ...
    it("Contract token balance is less than sum of all sending tokens", async () => {   
      await SimpleTokenInstance.transfer(PaymentBatcherInstance.address, depositAmount, {from: clientAccount1});
      expect(await SimpleTokenInstance.balanceOf(PaymentBatcherInstance.address)).to.be.bignumber.equal(depositAmount);
      return expect(PaymentBatcherInstance.transferTokensToMany([clientAccount1 + zeroFill(withdrAmount1.toString(16), 24) , clientAccount2 + zeroFill(withdrAmount2.toString(16), 24), clientAccount2 + zeroFill(withdrAmount3.toString(16), 24)], {from: ownerAccount})).to.be.eventually.rejected;
      });

    it("NOT owner cannot Batch tokens", async () => {   
      await SimpleTokenInstance.transfer(PaymentBatcherInstance.address, depositAmount, {from: clientAccount1});
      expect(await SimpleTokenInstance.balanceOf(PaymentBatcherInstance.address)).to.be.bignumber.equal(depositAmount);
      return expect(PaymentBatcherInstance.transferTokensToMany([clientAccount1 + zeroFill(withdrAmount1.toString(16), 24), clientAccount2 + zeroFill(withdrAmount3.toString(16), 24)], {from: clientAccount1})).to.be.eventually.rejected;
    });
});
